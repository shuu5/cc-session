...
- **marker は操作インスタンス単位**（例 `pr-merge-pr-3-sha-<head8>`）。対象や head SHA が変われば再 gate（「一度で永久解除」を防ぐ）。`marker_ttl_sec` で時間失効も設定可。`key.risk_flags`（allowlist）を持つ gate は危険フラグも区別する（例 pr-merge は `--admin` を `-flag-admin` として keying＝`--squash` の unlock が `--admin`〔レビュー要件 bypass〕を巻き込み認可しない・認可スコープ漏洩の防止）。
