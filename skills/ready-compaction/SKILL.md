...
# フィルタ無しの bd list を使う（closed のみ除外＝open/in_progress/blocked/deferred を全て含む。
# bd ready や --status=open は blocked/deferred を含まない subset のため取りこぼす）。
_root=$(git rev-parse --show-toplevel 2>/dev/null) || _root=$PWD
if [ -d "${_root}/.beads" ]; then bd list; fi
