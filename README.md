...
- **認可と unlock の分離が hard 性の核心**: gate 定義は `/session:enforce` が **LLM 提案 → 人間確定**で書く（authoring 専用）。実行時の許可は人間が生シェルで `scripts/enforce-unlock <gate> "<command>"`（または block 時に提示される `touch` コマンド）を叩いて marker を作る。**Claude は marker を作れない**＝自己認可不可。
