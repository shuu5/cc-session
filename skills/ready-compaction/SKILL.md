...
```bash
# 機械 fetch（決定論）。bd は .beads/ が存在するリポでのみ実行し、stderr は捨てない・
# exit code を確認する（「fetch 失敗」と「open が本当に無い」を区別するため）。
# bd ready は使わない（blocked/deferred/hooked を除外した subset のため open を取りこぼす）。
if [ -d .beads ]; then bd list --status=open; bd list --status=in_progress; fi
...
- 会話では「完了」と認識しているのに bd が open/in_progress → **bd 現在値を truth** とする。
