...
`/compact`（会話圧縮）とは別に、ユーザーが **`/clear`（文脈の完全リセット）してから Working Memory を読み込んで再開**したい運用がある。`/clear` は `SessionStart` を **`source: "clear"`** で発火させ（`compact` ではない・verified）、`PreCompact`/`PostCompact` は**発火させない**（compaction 専用）。よって `matcher: "compact"` だけの旧構成では `/clear` 後に**自動復元は一切走らない**（退避＝書き込みは動くが復元が繋がらない非対称）。
...
- **発見性フォールバック**: 厳密 sid 一致（`$WORKING_MEMORY_FILE`）が無ければ、ディレクトリ内で**最新 mtime の `working-memory*.md`**（`*.consumed.md` 除外）を提示する。`/clear` で session_id が変わると旧 sid 名のファイルは exact 一致しない（session-scoped 命名 un-gcu の副作用）ため、これで拾う。session_id が変わるかは **uncertain** だが、本設計は変化有無どちらでも動く防御設計。
- **un-gcu との両立**: cwd 共有の並走セッションがあると他セッションの退避ファイルを mtime で拾いうるが、**read-only なので un-gcu が閉じた「上書き破壊」は再導入しない**。代わりに「別セッション由来の可能性」を正直に明示しユーザー判断に委ねる。
